...
Almost everything in finance is feed-shaped: the Fed's own press releases, the
BEA, the SEC, the wires, the sell-side blogs, the newsletters. A dedicated
fetcher per outlet would be thirty near-identical files, so outlets live as
data in feeds.json and share this parser.

Feeds carry no popularity signal at all, and in this domain that matters less
than it sounds: a Federal Reserve statement has no upvotes and moves every
asset on earth. These items enter with points=0 and are ranked by `impact`
(publisher tier, policy language, cross-source agreement) and by the LLM's
read of the profile - which is the honest answer to "should I read this?"

`tier` comes from feeds.json and is the one piece of editorial judgement the
parser carries: tier 1 is a primary source publishing the thing itself, and
`impact` treats that very differently from a blog's reaction to it.
"""
...
class RSSFetcher:
    ...
    def __init__(self, name: str, url: str, tags=None, limit: int = 40,
                 timeout: int = 20, tier: int = 3):
        self.name = name
        self.url = url
