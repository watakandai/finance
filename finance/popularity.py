"""Turning incomparable crowd counts into one 0-100 number.

Deliberately a minor input in this project. Crowd attention is a real signal in
technology news; in finance it is mostly a lagging one, because the things that
move markets are published by institutions that have no vote count and read by
people who are not voting. So this produces a number, `impact` uses it as a
small bonus, and nothing here can carry an item on its own.

It still has to be done correctly where it applies. 1,900 upvotes on r/stocks
and 300 on r/SecurityAnalysis are not the same scale, do not have the same
distribution, and do not mean the same thing. Anything that compares them
directly - a fixed multiplier per source, a global sort on raw counts - is
measuring the size of a source's audience rather than how much attention an
item got.
...
SOURCE_TRUST = {
    "reddit": 0.9,
    "hackernews": 0.9,
    # A Seeking Alpha or StockTwits-style count, were one added, would sit
    # lower: the population is smaller and more directional.
}
...
def propagate(rows: list, scores: dict) -> dict:
    ...
    A Wall Street Journal piece that a subreddit also pushed to the top did get
    that attention - the feed just cannot see it. Without this, the crowd's copy
    ranks and the publisher's copy (the better-written one, with the chart) sits
    unranked at the bottom.
